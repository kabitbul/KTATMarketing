...
namespace KTSite.Areas.ExternalMerch.Controllers
{
    ...
    public class ProductStatisticsController : Controller
    {
        ...
        public ActionResult GetList()
        {
            ...
            var allWeekOrders = _unitOfWork.Order.GetAll().Where(a => a.OrderStatus != SD.OrderStatusCancelled && a.MerchType == SD.Role_ExMerch && a.MerchId == uNameId &&
        ...
        {
            string uNameid = returnUserNameId();
            List<Product> allProducts = _unitOfWork.Product.GetAll().Where(a=> a.MerchType == SD.Role_ExMerch && a.MerchId == uNameid).ToList();
        ...
        public void getStackGraphData(List<DataPoint> list, int ProductId)
        {
            ...
            string uNameId = returnUserNameId();
                var result = _unitOfWork.Order.GetAll().Where(a => a.MerchType == SD.Role_ExMerch && a.MerchId == uNameId &&
                                                              a.OrderStatus != SD.OrderStatusCancelled && a.ProductId == ProductId).
                    GroupBy(a => a.UsDate)
